...
class CustomerDashboardController extends Controller {
 ...
 public function vendor(Request $request){$vendor=Vendor::where('user_id',$request->user()->id)->first();$propertyCount=$vendor?->properties()->count() ?? 0;$bookingValue=$vendor?->properties()->withSum(['bookings'=>fn($q)=>$q->whereIn('status',['confirmed','completed'])],'total')->get()->sum('bookings_sum_total') ?? 0;return view('vendor.dashboard',compact('vendor','propertyCount','bookingValue'));}
}